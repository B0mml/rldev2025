...
-- @param b a value
-- @return a / b
M.operator.intdiv = function(a, b) return a >= 0 and floor(a / b) or ceil(a / b) end
...
-- @see best
-- @see reduceRight
function M.reduceBy(t, f, pred, state) return M.reduce(M.select(t, pred), f, state) end
...
-- @see best
-- @see reduceBy
function M.reduceRight(t, f, state) return M.reduce(M.reverse(t), f, state) end
...
-- @return an array of states
-- @see mapReduce
function M.mapReduceRight(t, f, state) return M.mapReduce(M.reverse(t), f, state) end
	...
	return M.map(t, function(v, k)
		if type(v) == "table" then
			if v[method] then
				...
				if M.isCallable(method) then return method(v, k) end
			end
...
-- @see min
function M.max(t, transform) return extract(t, f_max, transform) end
...
-- @see max
function M.min(t, transform) return extract(t, f_min, transform) end
...
function M.same(a, b)
	return M.all(a, function(v) return M.include(b, v) end) and M.all(b, function(v) return M.include(a, v) end)
end
...
function M.sortBy(t, transform, comp)
	...
	if type(transform) == "string" then f = function(t) return t[transform] end end
	comp = comp or f_min
...
-- @see addTop
-- @see push
function M.prepend(array, ...) return M.append({ ... }, array) end
...
function M.removeRange(array, start, finish)
	...
	if start > finish then error("start cannot be greater than finish.") end
...
function M.nth(array, index) return array[index] end
...
function M.difference(array, array2)
	...
	return M.select(array, function(value) return not M.include(array2, value) end)
end
...
-- @see intersection
-- @see symmetricDifference
function M.union(...) return M.unique(M.flatten({ ... })) end
...
function M.intersection(...)
	...
	for i, value in ipairs(array) do
		if M.all(arg, function(v) return M.include(v, value) end) then _intersect[#_intersect + 1] = value end
	end
	return _intersect
end
...
-- @see intersection
function M.disjoint(...) return (#M.intersection(...) == 0) end
...
-- @see union
-- @see intersection
function M.symmetricDifference(array, array2) return M.difference(M.union(array, array2), M.intersection(array, array2)) end
...
function M.unique(array)
	local ret = {}
	for i = 1, #array do
		if not M.find(ret, array[i]) then ret[#ret + 1] = array[i] end
	end
	return ret
end
...
-- @see unique
-- @see duplicates
function M.isunique(array) return #array == #(M.unique(array)) end
...
function M.duplicates(array)
	...
	local dups = {}
	for k, v in ipairs(array) do
		if dict[v] ~= k and not M.find(dups, v) then dups[#dups + 1] = v end
	end
	return dups
end
...
function M.partition(array, n, pad)
	if n <= 0 then return end
	return wrap(function() partgen(array, n or 1, yield, pad) end)
end
...
function M.overlapping(array, n, pad)
	...
	return wrap(function() partgen2(array, n or 2, yield, pad) end)
end
...
function M.aperture(array, n)
	if n <= 1 then return end
	return wrap(function() partgen3(array, n or 2, yield) end)
end
...
function M.permutation(array)
	return wrap(function() permgen(array, #array, yield) end)
end
...
-- @return a string
function M.concat(array, sep, i, j) return t_concat(M.map(array, tostring), sep, i, j) end
...
-- @see product
-- @see median
function M.mean(array) return M.sum(array) / #array end
...
function M.memoize(f)
	...
	return function(key)
		if _cache[key] == nil then _cache[key] = f(key) end
		return _cache[key]
	end
end
...
-- @see compose
function M.pipe(value, ...) return M.compose(...)(value) end
...
function M.bind2(f, v)
	return function(t, ...) return f(t, v, ...) end
end
...
function M.bindn(f, ...)
	local args = { ... }
	return function(...) return f(unpack(M.append(args, { ... }))) end
end
...
function M.cond(conds)
	return function(...)
		for k, condset in ipairs(conds) do
			if condset[1](...) then return condset[2](...) end
		end
	end
end
...
function M.castArray(value) return (type(value) ~= "table") and { value } or value end
...
-- @return a function
function M.flip(f)
	return function(...) return f(unpack(M.reverse({ ... }))) end
end
...
function M.converge(f, g, h)
	return function(...) return f(g(...), h(...)) end
end
...
function M.toBoolean(value) return not not value end
...
-- @return `true` or `false`
function M.has(obj, key) return obj[key] ~= nil end
...
function M.pick(obj, ...)
	...
	for key, property in pairs(whitelist) do
		if obj[property] ~= nil then _picked[property] = obj[property] end
	end
	return _picked
end
...
function M.omit(obj, ...)
	...
	local _picked = {}
	for key, value in pairs(obj) do
		if not M.include(blacklist, key) then _picked[key] = value end
	end
	return _picked
end
...
function M.result(obj, method)
	...
	if M.isCallable(method) then return method(obj) end
end
...
-- @return `true` or `false`
function M.isTable(t) return type(t) == "table" end
...
function M.isIterable(obj) return M.toBoolean((pcall(pairs, obj))) end
...
function M.type(obj)
	...
	if tp == "userdata" then
		...
		if stdout ~= nil and mt == getmetatable(stdout) then return "file" end
	end
	return tp
end
...
-- @param obj an object
-- @return `true` or `false`
function M.isString(obj) return type(obj) == "string" end

--- Checks if the given argument is a function.
-- @name isFunction
-- @param obj an object
-- @return `true` or `false`
function M.isFunction(obj) return type(obj) == "function" end

--- Checks if the given argument is nil.
-- @name isNil
-- @param obj an object
-- @return `true` or `false`
function M.isNil(obj) return obj == nil end
...
-- @return `true` or `false`
-- @see isNaN
function M.isNumber(obj) return type(obj) == "number" end
...
-- @return `true` or `false`
-- @see isNumber
function M.isNaN(obj) return type(obj) == "number" and obj ~= obj end
...
-- @param obj an object
-- @return `true` or `false`
function M.isBoolean(obj) return type(obj) == "boolean" end

--- Checks if the given argument is an integer.
-- @name isInteger
-- @param obj an object
-- @return `true` or `false`
function M.isInteger(obj) return type(obj) == "number" and floor(obj) == obj end
...
do
	...
	local function new(value) return setmetatable({ _value = value, _wrapped = true }, Moses) end
	...
	f.import = function(context, noConflict)
		...
		for k, fname in ipairs(funcs) do
			if rawget(context, fname) ~= nil then
				if not noConflict then rawset(context, fname, M[fname]) end
			else
